...
class MyBroadcastReceiver : BroadcastReceiver() {
    ...
    companion object {
        ...
        const val TOGGLE_WIFI = "com.example.sliceprovider.MyBroadcastReceiver.TOGGLE_WIFI"
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        } else if (intent?.action?.equals(DECREMENT_COUNTER_ACTION) == true && intent.hasExtra(
                EXTRA_VALUE_KEY
            )) {
            currentValue = intent.getIntExtra(EXTRA_VALUE_KEY, 0);
            context?.contentResolver?.notifyChange(dynamicSliceUri, null)
        } else if (intent?.action?.equals(INCREMENT_COUNTER_ACTION) == true && intent.hasExtra(
                EXTRA_VALUE_KEY
            )) {
            currentValue = intent.getIntExtra(EXTRA_VALUE_KEY, 0);
            context?.contentResolver?.notifyChange(dynamicSliceUri, null)
